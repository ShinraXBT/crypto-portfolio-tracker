  ...
  <div class="card">
    <div class="flex items-start justify-between">
      ...
      <div :class="['p-3 rounded-lg', colorClasses[color]]">
        <!-- Wallet Icon -->
        <svg v-if="icon === 'wallet'" class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
        </svg>
        <!-- Trophy Icon -->
        <svg v-else-if="icon === 'trophy'" class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
        </svg>
        <!-- Chart Down Icon -->
        <svg v-else-if="icon === 'chart-down'" class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 17h8m0 0V9m0 8l-8-8-4 4-6-6" />
        </svg>
        <!-- Bitcoin Icon -->
        <svg v-else-if="icon === 'bitcoin'" class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
